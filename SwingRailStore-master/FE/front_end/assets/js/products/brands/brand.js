main_app.controller("brandController", function ($scope, $http) {
  ...
  const loadData = function () {
    $http
      .get(
        "http://localhost:8080/brand/find-all-panigation?page=" +
          ($scope.currentPage - 1) +
          "&size=" +
          $scope.itemsPerPage +
          "&key=" +
  ...
  $scope.addBrand = function () {
    ...
    if ($scope.name_brand == "") {
      ...
      axios
        .post("http://localhost:8080/brand/add?name=" + $scope.name_brand)
        .then(function (response) {
        ...
        .catch(function (error) {
          toastr.error(
            "Tên thương hiệu này đã tồn tại.Vui lòng nhập tên thương hiệu khác!!!"
          );
        });
    }
  };
  ...
  $scope.loadBrand = function (id) {
    ...
    axios
      .get("http://localhost:8080/brand/get-brand/" + id)
      .then(function (response) {
      ...
      .catch(function (error) {
        console.log(error);
        toastr.error(
          "Đã có lỗi trong quá trình tải.Vui lòng liên hệ admin !!!"
        );
      });
  };
  ...
  $scope.updateBrand = function () {
    ...
    console.log($scope.brand);
    axios
      .put("http://localhost:8080/brand/update", $scope.brand)
      .then(function (response) {
        modal.hide();
