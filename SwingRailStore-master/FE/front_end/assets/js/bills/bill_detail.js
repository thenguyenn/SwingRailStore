main_app.controller(
  ...
  function ($scope, $http, $routeParams) {
    ...
    $scope.loadBillRefund = () => {
      $http
        .get(
          "http://localhost:8080/bill-detail/get-bill-detail-state?state=" +
            2 +
            "&id=" +
            $scope.bill.id
        )
        .then(function (response) {
          ...
          response.data.forEach((element) => {
            $scope.moneyRefund +=
              element.donGiaSauKhiGiam == null
                ? element.donGia * element.soLuong
                : element.donGiaSauKhiGiam * element.soLuong;
          });
        });
    ...
    $scope.loadBill = () => {
        ...
        .then(function (response) {
          ...
          for (var i = 0; i < $scope.billDetails.content.length; i++) {
            if ($scope.billDetails.content[i].donGiaSauKhiGiam == null) {
              $scope.bill.tongTien +=
                Number($scope.billDetails.content[i].donGia) *
                Number($scope.billDetails.content[i].soLuong);
            } else {
              $scope.bill.tongTien +=
                Number($scope.billDetails.content[i].donGiaSauKhiGiam) *
                Number($scope.billDetails.content[i].soLuong);
            }
          }
          ...
          if ($scope.bill.idVoucher != null) {
            $scope.bill.tongTienSauGiam =
              ((100 - $scope.bill.soPhanTramKhuyenMai) * $scope.bill.tongTien) /
              100;
          }

          $scope.bill.tongTienSauGiam += Number(
            $scope.bill.phiVanChuyen == null || $scope.bill.phiVanChuyen == ""
              ? 0
              : $scope.bill.phiVanChuyen
          );
        })
      ...
      $http
        .get("http://localhost:8080/history/get-all-by-id/" + id)
        .then(function (response) {
      ...
      $http
        .get("http://localhost:8080/payment-method/get-all/" + id)
        .then(function (response) {
    ...
    $scope.updateVoucher = function () {
      ...
      axios
        .put("http://localhost:8080/voucher/edit-voucher", $scope.voucher)
        .then((response) => {
    ...
    $scope.getFeeShipping = () => {
      ...
      axios
        ...
        .then((res) => {
          $scope.oldBill.tongTienSauGiam =
            $scope.bill.tongTien + res.data.data.total;
      ...
      fetch(
        ...
        .then((data) => {
          ...
          for (let i = 0; i < options.length; i++) {
            ...
            selectCityCustomer.appendChild(option);
          }
    ...
    $scope.getAllDistrictByCode = function (
      ...
      axios
        ...
        .then((res) => {
          ...
          for (let i = 0; i < options.length; i++) {
            const option = document.createElement("option");
            option.value = options[i].DistrictID;
            option.text = options[i].DistrictName;
            option.setAttribute("districtcode", options[i].DistrictID);
    ...
    $scope.successBill = function () {
      if ($scope.bill === null) {
        ...
        if ($scope.moneyChange < $scope.bill.tongTienSauGiam) {
          toastr.error(
            "Bạn phải xác nhận thanh toán trước khi hoàn thành đơn hàng."
          );
          return;
        }
    ...
    $scope.minusQuantity = function (billDetail) {
      if ($scope.moneyChange >= $scope.bill.tongTienSauGiam) {
        toastr.error(
          "Bạn không thể giảm số lượng khi tổng giá trị đơn hàng nhỏ hơn hoặc bằng số tiền thanh toán ."
        );
        return;
      }
    ...
    $scope.removeBillDetailByBillAndProductDetail = function (billDetailId) {
      ...
      axios
        .delete(
          "http://localhost:8080/bill-detail/remove-by-id/" + billDetailId
        )
        .then(function (response) {
          setTimeout(() => {
            $scope.addBill(
              `Hóa đơn ${$scope.bill.ma} đã thay đổi sản phẩm thành công`
            );
          }, 100);
        })
        .catch(function (response) {
        ...
        .then(function (response) {
          ...
          if ($scope.bill.idVoucher !== null) {
            if (
              ...
              $scope.bill.idVoucher.giaTriToiThieu
            ) {
              $scope.removeVoucher();
            }
          }
      ...
      if ($scope.bill === null) {
        ...
        setTimeout(() => {
          axios
            ...
            .catch(function (error) {});
          $scope.addBill(
            `Hóa đơn ${$scope.bill.ma} đã được hoàn trả hàng thành công`
          );
          $scope.loadBill();
        }, 100);
      }
    ...
    $scope.refundSingleProduct = (billDetail, quantity) => {
      if (billDetail.soLuong == quantity) {
        ...
        $http
          .post(
            "http://localhost:8080/bill-detail/add-product-to-bill-refund",
            {
          ...
          .then(function (response) {
            ...
            axios
              .put(
                "http://localhost:8080/bill-detail/refund-single",
                billDetail
              )
              .then((response) => {})
              .catch((error) => {
    ...
    $scope.refundSingleBill = () => {
      ...
      if ($scope.bill === null) {
        ...
        var brandUpdateModal = document.querySelector(
          "#billDetailSingleRefund"
        );
        var modal = bootstrap.Modal.getOrCreateInstance(brandUpdateModal);
        ...
        $scope.bill.trangThai = 6;
        $scope.updateStateOfBill($scope.bill.trangThai, $scope.noteState4);
        $scope.refundSingleProduct(
          $scope.billDetailRefund,
          $scope.quantityRefund
        );
    ...
    $scope.choosePaymentMethod = function (paymentMethod) {
      ...
      var htmls = document.getElementsByClassName(
        "payment-product-detail-" + paymentMethod
      );
      htmls.forEach((e) => {
      ...
      axios
        ...
        .then(function (response) {
          ...
          setTimeout(() => {
            ...
            alert("Hóa đơn" + $scope.bill.ma + "đã hoàn tiền thành công ");
            $scope.loadBill();
            window.location.reload();
          }, 100);
        })
    ...
    $scope.backPayment = () => {
      ...
      axios
        ...
        .then(function (response) {
          ...
          setTimeout(() => {
            $scope.addBill(
              `Hóa đơn ${$scope.bill.ma} đã hoàn tiền thành công.`
            );
            $scope.loadBill();
          }, 100);
        })
