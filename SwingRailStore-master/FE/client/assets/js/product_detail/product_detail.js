clientApp.controller(
  ...
  function ($scope, $http, $routeParams) {
    ...
    $scope.loadSizes = () => {
      $http.get("http://localhost:8080/size/get-all").then(function (response) {
        $scope.sizes = response.data;
      });

      if (
        $scope.productChooseCurrent != null &&
        $scope.productChooseCurrent != undefined
      ) {
        if ($scope.customer != null) {
          $http
            .get(
              "http://localhost:8080/cart/get-cart-detail-by-id-cart?id_customer=" +
                $scope.customer.id +
                "&id_product_detail=" +
                $scope.productChooseCurrent.id
            )
            .then((response) => {
    ...
    $scope.loadData = function () {
      ...
      $http
        .get(
          "http://localhost:8080/product-detail/find-all-panigation?page=" +
            ($scope.currentPageProductDetail - 1) +
            "&size=" +
            $scope.itemsPerPageProductDetail +
            "&trang_thai=" +
            "&idSize=" +
            "&idColor=" +
            "&id=" +
      ...
      $http
        .get("http://localhost:8080/product-detail/get-color-identity/" + id)
        .then(function (response) {
      ...
      $http
        .get("http://localhost:8080/product-detail/get-product-newests")
        .then((response) => {
    ...
    $scope.clearMarkColor = () => {
      var colorHtml = document
        .getElementById("color-product-detail")
    ...
    $scope.clearMarkSize = () => {
      var sizeHtml = document
        .getElementById("size-product-detail")
        .getElementsByTagName("button");
    ...
    $scope.chooseColor = (id) => {
      ...
      $scope.productDetailChooses = $scope.productDetails.filter(
        (productDetail) => productDetail.idMauSac.id === id
      );
      $scope.loadSizes();
    ...
    $scope.chooseSize = (productDetail) => {
      ...
      // active current choose color
      var currentSize = document.getElementById(
        "size-product-detail-" + productDetail.idKichCo.id
      );
      ...
      setTimeout(() => {
        $scope.loadCartDetails();
        $scope.getAllImagesBecomeSlides(
          $scope.productChooseCurrent.id,
          "product"
        );
      }, 100);
    ...
    $scope.addToCart = () => {
      ...
      if ($scope.cartDetails.length != 0) {
        ...
        if (
          $scope.productChooseCurrent.soLuongTon <
          $scope.quantity_of_cart + $scope.cartDetails[0].soLuong
        ) {
          toastr.error(
            "Số lượng còn lại trong kho không đủ.Vui lòng chọn sản phẩm khác."
          );
          return;
        }
      } else {
      ...
      $http
        .post("http://localhost:8080/cart/add-to-cart-quantity", {
