clientApp.controller("checkoutController", function ($scope, $http, $window) {
  // id
  $scope.cartDetails = [];
  ...
  $scope.loadDataProductDetail = () => {
    ...
    $http
      .get("http://localhost:8080/cart/get-cart-detail-by-id/" + id)
      .then(function (response) {
        $scope.cartDetails = response.data;
        $scope.subTotal = 0;
        for (var i = 0; i < $scope.cartDetails.length; i++) {
          $scope.subTotal +=
            $scope.cartDetails[i].idSanPhamChiTiet.idDotGiamGia == null
              ? $scope.cartDetails[i].soLuong *
                $scope.cartDetails[i].idSanPhamChiTiet.donGia
              : (($scope.cartDetails[i].soLuong *
                  (100 -
                    $scope.cartDetails[i].idSanPhamChiTiet.idDotGiamGia
                      .phanTramGiam)) /
                  100) *
                $scope.cartDetails[i].idSanPhamChiTiet.donGia;
        }
        ...
        if (
          $scope.bill.phiVanChuyen !== null &&
          $scope.bill.phiVanChuyen !== undefined
        ) {
        ...
        if ($scope.bill.idVoucher != null) {
          $scope.total -= Number(
            ($scope.bill.idVoucher.phanTramGiam * $scope.subTotal) / 100
          );
        }
      })
          ...
          .then(function (response) {
            ...
            $scope.voucherDetails.forEach((e) => {
              if (e.idPhieuGiamGia.giaTriToiThieu <= $scope.total) {
                if (
                  temp.idPhieuGiamGia.phanTramGiam <
                  e.idPhieuGiamGia.phanTramGiam
                ) {
                  temp = e;
                }
              }
  ...
  $scope.getNewBill = () => {
    $http
      .get("http://localhost:8080/bill/get-new-bill")
      .then(function (response) {
  ...
  $scope.checkout = () => {
    ...
    $scope.cartDetails.forEach((x, index) => {
      $http
        .post("http://localhost:8080/bill-detail/check-bill-detail-client", {
          hoaDon: $scope.bill,
        ...
        if (result.isConfirmed) {
          ...
          if ($scope.paymentMethod == 0) {
            axios
              ...
              .then(function (response) {
                ...
                $scope.cartDetails.forEach((x, index) => {
                  $http
                    .post(
                      "http://localhost:8080/bill-detail/add-bill-detail-client",
                      {
                        hoaDon: response.data,
                        sanPhamChiTiet: x.idSanPhamChiTiet,
                        soLuong: x.soLuong,
            ...
            localStorage.setItem(
              "bill_detail_vnpay",
              JSON.stringify($scope.cartDetails)
            );
            var currentLocation = "/";
            axios
              .post(
                "http://localhost:8080/payment?total=" +
                  Math.round($scope.total) +
                  "&orderInfor=" +
                  $scope.bill.ma +
                  "&currentLocation=" +
                  currentLocation
  ...
  $scope.checkVoucher = () => {
    ...
    $http
      .get(
        "http://localhost:8080/voucher/get-voucher-absolute?id_voucher=" +
          $scope.codeVoucher
      )
      .then(function (response) {
        // if(response.data.)
        if (response.data.trangThai !== 1) {
          toastr.error(
            "Mã phiếu giảm giá đã hết hạn.Vui lòng chọn mã phiếu khác!!!"
          );
          return;
        }
        ...
        if (response.data.giaTriToiThieu > $scope.subTotal) {
          toastr.error(
            "Giá trị hóa đơn không đủ điều kiện để sử dụng mã phiếu giảm giá này."
          );
          return;
        }
