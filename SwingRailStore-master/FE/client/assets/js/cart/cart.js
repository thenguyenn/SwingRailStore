clientApp.controller("cartController", function ($scope, $http) {
  // id
  $scope.cartDetails = [];
  $scope.subTotal = 0;
  ...
  $scope.removeCartDetail = (e) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .delete(
            "http://localhost:8080/cart-detail/remove-cart-detail/" + e.id
          )
          .then(function (response) {
  ...
  $scope.minusQuantity = (cartDetail) => {
    if (cartDetail.soLuong > 1) {
      ...
      axios
        .delete(
          "http://localhost:8080/cart-detail/remove-cart-detail/" +
            cartDetail.id
        )
        .then(function (response) {
          window.location.reload();
